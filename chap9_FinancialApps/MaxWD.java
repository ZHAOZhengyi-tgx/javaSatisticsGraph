package chap9_FinancialApps;

/* Compute the initial investment necessary for
a desired annuity. In other words, find
...
double b, e;
double t1, t2;
t1 = rateOfRet/numPerYear; // (regWDAmount * numPerYear) / rateOfRet;
b = (1 + t1); // (1 + rateOfRet/numPerYear);
e = numPerYear * numYears;
t2 = Math.pow(b, e) - 1;  // 1 - (1 / Math.pow(b, e));
return regWDAmount *(t1/t2 + t1); // t1 * t2;
}
}
